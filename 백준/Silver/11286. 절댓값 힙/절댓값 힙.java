        ...
        for (int i = 0; i < n; i++) {
            ...
            if (m == 0) {
                ...
                while (idx * 2 <= heapSize) {
                    ...
                    if (idx * 2 + 1 <= heapSize 
                    	&& (Math.abs(minHeap[idx * 2]) > Math.abs(minHeap[idx * 2 + 1])
                    		|| (Math.abs(minHeap[idx * 2]) == Math.abs(minHeap[idx * 2 + 1]) 
                    			&& minHeap[idx * 2] > minHeap[idx * 2 + 1]
                    			)
                    		)
                    	) {
                        leftOrRight += 1;
                    }
                    if (Math.abs(minHeap[idx]) > Math.abs(minHeap[leftOrRight])
                    		|| (Math.abs(minHeap[idx]) == Math.abs(minHeap[leftOrRight]) 
                ...
                int idx = heapSize;
                while (idx > 1
                		&& (Math.abs(minHeap[idx/2]) > Math.abs(minHeap[idx])
                        		|| (Math.abs(minHeap[idx/2]) == Math.abs(minHeap[idx]) 
